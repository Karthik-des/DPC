...
const SearchForm = ({ navigation, route }) => {
  ...
  const fetchSuggestions = async (query, inputType) => {
    ...
    try {
      ...
      Alert.alert('Error', 'Failed to fetch location suggestions. Using fallback suggestions.');
  ...
  const handleSearch = () => {
    if (!fromLocation || !toLocation) {
      Alert.alert('Missing Fields', 'Please fill in both Leaving from and Going to fields.');
      return;
    }
    const params = {
  ...
  const renderItem = ({ item }) => {
    switch (item.type) {
      ...
      case 'fromInput':
        return (
          <Animated.View entering={FadeIn.duration(300)} style={styles.inputWrapper}>
            <Ionicons name="location" size={28} color="#09C912" style={styles.icon} />
            <TextInput
              style={styles.input}
            ...
            />
            {fromLocation && (
              <TouchableOpacity onPress={() => clearInput('from')} style={styles.clearButton}>
      ...
      case 'fromSuggestions':
        return activeInput === 'from' && (isLoadingSuggestions ? (
      ...
      case 'toInput':
        return (
          <Animated.View entering={FadeIn.duration(300)} style={styles.inputWrapper}>
            ...
            />
            {toLocation && (
              <TouchableOpacity onPress={() => clearInput('to')} style={styles.clearButton}>
                <Ionicons name="close" size={20} color="#666" />
              </TouchableOpacity>
            )}
          </Animated.View>
      ...
      case 'toSuggestions':
        return activeInput === 'to' && (isLoadingSuggestions ? (
          <ActivityIndicator size="small" color="#09C912" style={styles.loader} />
      ...
      case 'addInput':
        return (
          <Animated.View entering={FadeIn.duration(300)} style={styles.inputWrapper}>
            <Ionicons name={addType === 'Add' ? 'add' : 'remove'} size={28} color="#09C912" style={styles.icon} />
            <TextInput
              style={styles.input}
      ...
      case 'addSuggestions':
        return activeInput === 'add' && (isLoadingSuggestions ? (
          <ActivityIndicator size="small" color="#09C912" style={styles.loader} />
          ...
          <Animated.View entering={FadeIn.duration(300)} style={styles.suggestionContainer}>
            <FlatList
              ...
              renderItem={({ item }) => (
                <TouchableOpacity
                  style={styles.suggestionItem}
                  onPress={() => handleSuggestionSelect(item, 'add')}
                >
                  <Text style={styles.suggestionText}>{item.label}</Text>
                </TouchableOpacity>
              )}
      ...
      case 'locationPassengerInput':
        return addLocation && addType === 'Add' ? (
          <Animated.View entering={FadeIn.duration(300)} style={[styles.inputWrapper, styles.passengerWrapper]}>
